...
export default function SlotAvailability({
  ...
  const [selectedConsultant, setSelectedConsultant] = useState<any>(null); // null = "Any available", object = specific consultant
  ...
  // Get all consultants (for name lookup when "Any available" is selected)
  const allConsultants = React.useMemo(() => {
    if (!consultantsData?.users?.data) return [];
    return consultantsData.users.data;
  }, [consultantsData]);

  // Filter consultants based on session type (for selection modal)
  const consultants = React.useMemo(() => {
    if (!consultantsData?.users?.data) return [];
    
    const filtered = consultantsData.users.data.filter((consultant: any) => {
    ...
    console.log('✅ Filtered consultants for', sessionType, ':', filtered.map((c: any) => ({
      name: `${c.profileData?.firstName} ${c.profileData?.lastName}`,
      allowOnlineBooking: c.profileData?.allowOnlineBooking,
      allowOnlineDelivery: c.profileData?.allowOnlineDelivery
    })));
    
    return filtered;
  }, [consultantsData, sessionType]);

  // Use available slots hook - show all slots by default, filter by consultant when selected
  // When "Any available" is selected (selectedConsultant is null), pass all consultants for name lookup
  // but filtered ones for slot generation
  const { availableSlots, loading: slotsLoading } = useAvailableSlots({
    ...
    isReturningUser: selectedConsultant ? !isNewUser : false, // Treat as new user when "Any available" to get per-consultant slots
    ...
    allConsultantsForLookup: selectedConsultant ? undefined : allConsultants, // Pass all consultants for lookup when "Any available"
  });
  ...
  useEffect(() => {
    ...
    const processedSlots = availableSlots
      .filter(slot => {
        // Only include slots from consultants in the filtered list
        if (!slot.consultantId) return false;
        const isInFilteredList = consultants.some((c: any) => c._id === slot.consultantId);
        if (!isInFilteredList) {
          console.log('🚫 Filtering out slot for consultant:', slot.consultantId);
        }
        return isInFilteredList;
      })
      .map(slot => {
        // Find consultant from filtered list only
