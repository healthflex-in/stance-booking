...
export default function PrepaidNewSlotSelection({ serviceDuration, designation, onSlotSelect, onBack }: PrepaidNewSlotSelectionProps) {
  ...
  useEffect(() => {
    ...
    if (!selectedDate && initialDates.length > 0) {
      ...
      setSelectedDate(`${firstDate.day}, ${firstDate.date} ${firstDate.month}`);
  ...
  useEffect(() => {
    ...
    const processedSlots = Array.from(slotMap.values()).sort((a, b) => new Date(a.startTimeRaw).getTime() - new Date(b.startTimeRaw).getTime());
    setDateSlots(prev => ({ ...prev, [dateKey]: processedSlots }));
    setAvailableDates(prev => prev.map(date => date.fullDate.toDateString() === currentSelectedDate.toDateString() ? { ...date, slots: processedSlots.length } : date));
  ...
  return (
    <div className={`${isInDesktopContainer ? 'h-full' : 'min-h-screen'} bg-gray-50 flex flex-col`}>
      <div className="flex-1 overflow-y-auto">
        <div className={`p-4 ${isInDesktopContainer ? 'pb-6' : 'pb-32'}`}>
          <div className="mb-6">
            ...
            <div className="mb-4">
              <div ref={scrollContainerRef} className="flex overflow-x-auto space-x-3 pb-2" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                {availableDates.map((dateOption) => {
                  const dateKey = `${dateOption.day}, ${dateOption.date} ${dateOption.month}`;
                  const isCurrentDate = selectedDate === dateKey;
                  return (
                    <button key={`date-${dateOption.fullDate.getTime()}`} onClick={() => handleDateSelect(dateOption)} disabled={slotsLoading && !isCurrentDate} className={`flex-shrink-0 p-3 rounded-xl border-2 transition-all text-center min-w-[120px] ${isCurrentDate ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                      <div className={`text-sm font-medium mb-1 ${isCurrentDate ? 'text-blue-700' : dateOption.isToday ? 'text-blue-600' : 'text-gray-900'}`}>
                        {dateOption.day}, {dateOption.date} {dateOption.month}
                      </div>
                      <div className={`text-xs ${typeof dateOption.slots === 'number' && dateOption.slots === 0 ? 'text-red-500' : isCurrentDate ? 'text-blue-600' : 'text-gray-500'}`}>
                        {typeof dateOption.slots === 'number' ? (dateOption.slots === 0 ? '0 slots' : `${dateOption.slots} slots`) : dateOption.slots}
                      </div>
                    </button>
                  );
                })}
      ...
      <div className={`${isInDesktopContainer ? 'flex-shrink-0' : 'fixed bottom-0 left-0 right-0'} bg-white border-t border-gray-200 p-4`}>
        <button onClick={handleContinue} disabled={!selectedTimeSlot} className={`w-full py-4 rounded-2xl font-semibold text-black transition-all ${selectedTimeSlot ? '' : 'bg-gray-300 cursor-not-allowed'}`} style={{ backgroundColor: selectedTimeSlot ? '#DDFE71' : '#D1D5DB' }}>
          Continue
        </button>
      </div>
    </div>
